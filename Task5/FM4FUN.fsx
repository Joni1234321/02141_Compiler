...
//    printfn "%A" pm
    ...
    printfn "SPF\n%s" (List.foldBack (fun (q0,p,q1) s -> sprintf "(%d, %s, %d) \n %s " q0 (printPredicate p) q1 s) reducedSPF "")
    

    let signs = 
        if name = "Factorial" then signAnalysis pg ["x", Positive; "y", Positive]
        else (Map.empty<int, Set<PowerSet>>)

    printfn "Sign analysis: %A" signs
